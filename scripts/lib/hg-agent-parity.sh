...
hg_parity_render_codex_ollama_profiles() {
  ...
  cat <<EOF
$(hg_parity_codex_ollama_start_marker)# Generated by codexkit/scripts/provider-settings-sync.sh from dotfiles shared local-model defaults.
...
hg_parity_upsert_generated_block_text() {
  ...
  local end_marker="$3"
  local block="$4"
  local tmpdir input_path output_path
  ...
  if grep -Fqx "$start_marker" "$input_path"; then
    awk \
