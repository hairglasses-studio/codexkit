...
func TestSync_RejectsInvalidToolSelections(t *testing.T) {
	tests := []struct {
		name     string
		enabled  []string
		disabled []string
		...
		{name: "duplicate enabled", enabled: []string{"status", "status"}, want: "duplicate"},
		{name: "enabled and disabled", enabled: []string{"status"}, disabled: []string{"status"}, want: "both enabled and disabled"},
	}
	tooMany := make([]string, maxEnabledTools+1)
	for i := range tooMany {
	...
	tests = append(tests, struct {
		...
		disabled []string
		want     string
	}{name: "too many enabled", enabled: tooMany, want: "at most 512"})
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			policyJSON, err := json.Marshal(map[string]any{
				...
				"profiles": []map[string]any{{
					"name":           "github",
					"from":           "github",
					"enabled_tools":  tt.enabled,
					"disabled_tools": tt.disabled,
				}},
			})
			if err != nil {
				t.Fatal(err)
			...
			writeFile(t, dir, ".codex/mcp-profile-policy.json", string(policyJSON))
			report := Sync(dir, true)
			if len(report.Errors) != 1 || !strings.Contains(report.Errors[0], tt.want) {
				t.Fatalf("errors = %v, want substring %q", report.Errors, tt.want)
			}
		})
	}
}
