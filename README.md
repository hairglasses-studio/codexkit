...
Repo-local wrapper scripts (`scripts/run-codexkit-mcp.sh`, `scripts/skill-surface-sync.sh`, `scripts/codex-mcp-sync.sh`) now source `~/hairglasses-studio/scripts/go-workspace-env.sh` when it is available. By default they reuse the shared `~/.cache/go-build` and stage temp work under `~/.cache/hairglasses-studio/go-tmp/codexkit` instead of `/tmp`.
